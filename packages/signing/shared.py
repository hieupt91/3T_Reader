...
def validate_signed_pdf_status(path: str) -> dict[str, object]:
    """Return a detailed post-signing integrity summary for the last signature."""
    try:
        ...
        with open(path, "rb") as f:
            ...
            if isinstance(public_key_bits, int) and public_key_bits and public_key_bits < 2048:
                policy_warning = (
                    f"Khóa RSA {public_key_bits} bit, thấp hơn khuyến nghị 2048 bit."
                )

            try:
                permissive_policy = DisallowWeakAlgorithmsPolicy(rsa_key_size_threshold=0)
                import asyncio

                status = asyncio.run(
                    async_validate_pdf_signature(
                        embedded_sig,
                        algorithm_policy=CMSAlgorithmUsagePolicy.lift_policy(permissive_policy),
                    )
                )
            ...
            return {
                "ok": overall_ok and (trusted or signing_time_ok is True or signing_time_ok is None),
                ...
                "validation_error": validation_error or policy_warning,
