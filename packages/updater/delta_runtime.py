...
def _launch_deelevated(executable: str, cwd: str) -> None:
    """Relaunch ``executable`` at the user's normal (non-elevated) integrity
    level even though this helper process itself is running elevated (UAC).

    ``subprocess.Popen()`` here would inherit our elevated token, leaving
    the GUI app running as Administrator after every delta update - a real
    bug confirmed via live QA testing (15/08/2026): an elevated GUI window
    silently blocks normal (non-elevated) automation/scripting from
    interacting with it at all (Windows UIPI: SetForegroundWindow,
...
def run_apply_helper_from_argv(argv: list[str]) -> bool | None:
    ...
    try:
        ...
        if applied:
            _launch_deelevated(argv[4], str(Path(argv[4]).resolve().parent))
        return applied
    except Exception:
        return False
