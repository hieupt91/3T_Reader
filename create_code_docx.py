...
def create_source_code_docx():
    ...
    p = doc.add_paragraph("MÃ NGU?N PH?N M?M 3T READER\n(50 TRANG Ð?U VÀ 50 TRANG CU?I)")
    ...
    p = doc.add_paragraph("--- 50 TRANG Ð?U TIÊN ---\n")
    p.runs[0].bold = True
    doc.add_paragraph("".join(first_lines))
    
    if last_lines:
        doc.add_page_break()
        p = doc.add_paragraph("--- 50 TRANG CU?I CÙNG ---\n")
        p.runs[0].bold = True
